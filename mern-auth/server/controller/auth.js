...
exports.acccountActivation =(req,res)=>{
...
//requireSignin
//validate token
//this will on the basis of valid token makes the data available by name of req.user
...
//             User.findOne({email}).exec((err,user)=>{
//                 if(user){
//                     const token = jwt.sign({_id:user._id},process.env.JWT_SECRET,{expiresI:'7d'});
...
//                         }
//                         const token = jwt.sign({_id:data._id},process.env.JWT_SECRET,{expiresI:'7d'});
//                         const {_id,email,name,role} = username
//                         return res.json({
//                         token,user:{_id,email,name,role}
//                     })
